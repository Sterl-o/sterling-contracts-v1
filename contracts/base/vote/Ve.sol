...
contract Ve is IERC721, IERC721Metadata, IVe, Reentrancy {
  ...
  string constant public override name = "veSTR";
  string constant public override symbol = "veSTR";
  ...
  function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) internal pure returns (string memory output) {
    ...
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', _toString(_tokenId), '", "description": "Sterling locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
