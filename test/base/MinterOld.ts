...
import {Controller, Str, StrMinter, Token, Ve, VeDist } from "../../typechain";
...
describe("minter old tests", function () {
  ...
  it("deploy base", async function () {
    ...
    ve_underlying = await Str.deploy();
    ...
    const factory = await StrFactory.deploy(treasury.address);
    await factory.deployed();
    const StrRouter01 = await ethers.getContractFactory("StrRouter01");
    const router = await StrRouter01.deploy(factory.address, owner.address);
    ...
    const voter = await StrVoter.deploy(ve.address, factory.address, gauges_factory.address, bribe_factory.address);
  ...
  it("initialize veSTR", async function () {
