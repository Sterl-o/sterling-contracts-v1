...
describe("pair tests", function () {
  ...
  it("insufficient liquidity minted revert", async function () {
    await expect(pair2.mint(owner.address)).revertedWith('StrPair: INSUFFICIENT_LIQUIDITY_MINTED');
  });

  it("insufficient liquidity burned revert", async function () {
    await expect(pair2.burn(owner.address)).revertedWith('StrPair: INSUFFICIENT_LIQUIDITY_BURNED');
  });
  ...
  it("insufficient output amount", async function () {
    await expect(pair2.swap(0, 0, owner.address, '0x')).revertedWith('StrPair: INSUFFICIENT_OUTPUT_AMOUNT');
  });

  it("insufficient liquidity", async function () {
    await expect(pair2.swap(Misc.MAX_UINT, Misc.MAX_UINT, owner.address, '0x')).revertedWith('StrPair: INSUFFICIENT_LIQUIDITY');
  });

  it("invalid to", async function () {
    await expect(pair2.swap(1, 1, wmatic.address, '0x')).revertedWith('StrPair: INVALID_TO');
  });
  ...
  it("insufficient input amount", async function () {
    await expect(pair2.swap(10000000, 1000000, owner.address, '0x')).revertedWith('StrPair: INSUFFICIENT_INPUT_AMOUNT');
  });

  it("k revert", async function () {
    await mim.transfer(pair2.address, 1);
    await expect(pair2.swap(10000000, 1000000, owner.address, '0x')).revertedWith('StrPair: K');
  });
...
async function checkTwap(pair: StrPair, tokenIn: string, diff: BigNumber) {
