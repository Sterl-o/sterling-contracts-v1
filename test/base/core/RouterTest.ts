...
describe("router tests", function () {
  ...
  before(async function () {
    ...
    factory = await Deploy.deployStrFactory(owner, owner.address);
    router = await Deploy.deployStrRouter01(owner, factory.address, wmatic.address);
  ...
  it("removeLiquidityMATIC test", async function () {
    ...
    await StrPair__factory.connect(pairAdr, owner).approve(router.address, parseUnits('1111'));
  ...
  it("removeLiquidityWithPermit test", async function () {
    ...
    const pairAdr = await factory.getPair(mim.address, wmatic.address, true);
    const pair = StrPair__factory.connect(pairAdr, owner);
  ...
  it("deadline reject", async function () {
    await expect(router.addLiquidityMATIC(
      ...
      1,
      {value: parseUnits('10')}
    )).revertedWith('StrRouter: EXPIRED');
  });
  ...
  it("sort tokens identical address", async function () {
    await expect(router.sortTokens(
      mim.address,
      mim.address,
    )).revertedWith('StrRouter: IDENTICAL_ADDRESSES');
  });
  ...
  it("sort tokens zero address", async function () {
    await expect(router.sortTokens(
      mim.address,
      Misc.ZERO_ADDRESS,
    )).revertedWith('StrRouter: ZERO_ADDRESS');
  });
  ...
  it("receive matic not from wmatic reject", async function () {
    await expect(owner.sendTransaction({value: 1, to: router.address})).revertedWith("StrRouter: NOT_WMATIC");
  });
  ...
  it("getAmountsOut wrong path", async function () {
    await expect(router.getAmountsOut(0, [])).revertedWith('StrRouter: INVALID_PATH');
  });

  it("quoteLiquidity zero amount", async function () {
    await expect(router.quoteLiquidity(0, 0, 0)).revertedWith('StrRouter: INSUFFICIENT_AMOUNT');
  });

  it("quoteLiquidity IL", async function () {
    await expect(router.quoteLiquidity(1, 0, 0)).revertedWith('StrRouter: INSUFFICIENT_LIQUIDITY');
  });
  ...
  it("add liquidity amount desired check", async function () {
    await expect(router.addLiquidityMATIC(
      ...
      99999999999,
      {value: parseUnits('10')}
    )).revertedWith('StrRouter: DESIRED_A_AMOUNT');
    await expect(router.addLiquidityMATIC(
      mim.address,
      true,
      parseUnits('1'),
      ...
      99999999999,
      {value: parseUnits('10')}
    )).revertedWith('StrRouter: DESIRED_B_AMOUNT');
  });
  ...
  it("add liquidity IA check", async function () {
    ...
    )).revertedWith('StrRouter: INSUFFICIENT_B_AMOUNT');
    ...
    )).revertedWith('StrRouter: INSUFFICIENT_A_AMOUNT');
  });
  ...
  it("swapExactTokensForTokensSimple IOA test", async function () {
    ...
    )).revertedWith('StrRouter: INSUFFICIENT_OUTPUT_AMOUNT');
  });
  ...
  it("swapExactMATICForTokens IP test", async function () {
    await expect(router.swapExactMATICForTokens(
      ...
      owner.address,
      BigNumber.from('999999999999999999'),
    )).revertedWith('StrRouter: INVALID_PATH');
  });
  ...
  it("router with broken matic should revert", async function () {
    ...
    const routerWithBrokenMatic = await Deploy.deployStrRouter01(owner, factory.address, brokenMatic.address);
    ...
    await expect(routerWithBrokenMatic.swapExactTokensForMATIC(
      ...
      owner.address,
      99999999999
    )).revertedWith('StrRouter: ETH_TRANSFER_FAILED');
  });
