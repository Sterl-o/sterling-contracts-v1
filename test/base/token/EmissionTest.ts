...
describe("emission tests", function () {
  ...
  it("update period 10 weeks", async function () {
    ...
    expect(await core.token.balanceOf(core.minter.address)).is.eq(0);
    // not exact amount coz veSTR balance fluctuation during time
  ...
  it("update period and distribute reward to voter and veDist", async function () {
    ...
    expect(await core.token.balanceOf(owner.address)).is.eq(0, "before the first update we should have 0 STR");
    const veBalance = (await core.ve.locked(1)).amount;

    await core.veDist.claim(1);

    // claimed STR will be deposited to veSTR
...
async function emissionLoop(
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const log = receipt.events?.find((l: any) => l.topics[0] === StrMinter__factory.createInterface().getEventTopic('Mint'));
    let weekly = '-1';
    let growth = '-1';
    if (log) {
      weekly = formatUnits(StrMinter__factory.createInterface().parseLog(log).args[1]);
      growth = formatUnits(StrMinter__factory.createInterface().parseLog(log).args[2]);
    }
