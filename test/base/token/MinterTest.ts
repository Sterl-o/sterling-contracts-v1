...
import {Controller, StrPair, Token} from "../../../typechain";
...
describe("minter tests", function () {
  ...
  it("wrong total amount test", async function () {
    ...
    const baseFactory = await Deploy.deployStrFactory(owner, treasury.address);
    ...
    const voter = await Deploy.deployStrVoter(owner, ve.address, baseFactory.address, gaugesFactory.address, bribesFactory.address);
    await controller.setVeDist(veDist.address)
    await controller.setVoter(voter.address)
    const minter = await Deploy.deployStrMinter(owner, ve.address, controller.address, 1);
