...
describe("token tests", function () {
  ...
  it("set minter reject", async function () {
    await expect(core.token.setMinter(Misc.ZERO_ADDRESS)).revertedWith('STR: Not minter')
  });

  it("approve reject", async function () {
    await expect(core.token.approve(Misc.ZERO_ADDRESS, 0)).revertedWith('STR: Approve to the zero address')
  });

  it("mint to zero address reject", async function () {
    const minter = await Misc.impersonate(await core.token.minter());
    await expect(core.token.connect(minter).mint(Misc.ZERO_ADDRESS, 1)).revertedWith('STR: Mint to the zero address')
  });

  it("transfer to zero address reject", async function () {
    await expect(core.token.transfer(Misc.ZERO_ADDRESS, 1)).revertedWith('STR: Transfer to the zero address')
  });

  it("transfer to too much reject", async function () {
    await expect(core.token.transfer(owner2.address, Misc.MAX_UINT)).revertedWith('STR: Transfer amount exceeds balance')
  });
  ...
  it("transfer from to too much reject", async function () {
    ...
    await expect(core.token.transferFrom(owner2.address, owner.address, 1)).revertedWith('STR: Insufficient allowance')
  });

  it("mint from not minter reject", async function () {
    await expect(core.token.mint(Misc.ZERO_ADDRESS, 1)).revertedWith('STR: Not minter')
  });
