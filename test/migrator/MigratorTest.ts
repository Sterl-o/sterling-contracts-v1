...
describe("migrator tests", function () {
  ...
  it("migrate basic test", async function () {
    ...
    const {
      ...
      owner,
      StrPair__factory.connect(oldPair.address, owner),
      migrator.address,
      oldPairBalance
    );
    ...
    const bal = await StrPair__factory.connect(pair, owner).balanceOf(owner.address);
    ...
    const bal2 = await StrPair__factory.connect(pair, owner).balanceOf(owner.address);

    const ustBal = await ust.balanceOf(owner.address);
    const mimBal = await mim.balanceOf(owner.address);
    await StrPair__factory.connect(pair, owner).transfer(pair, bal2);
    await StrPair__factory.connect(pair, owner).burn(owner.address);
    ...
    expect(await StrPair__factory.connect(pair, owner).balanceOf(migrator.address)).is.eq(0);
